...
def _serialize_value(value):
    ...
    if isinstance(value, set):
        return sorted(_serialize_value(v) for v in value)  # sets não são serializáveis em JSON
...
def as_response_models(
    ...
    """
    return [as_response_model(instance, exclude=exclude, include=include) for instance in model_instances if instance is not None]
