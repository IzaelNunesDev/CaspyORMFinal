...
class Tuple(BaseField):
    ...
    def __init__(self, *field_types: BaseField, **kwargs):
        ...
        for field_type in field_types:
            if not isinstance(field_type, BaseField):
                raise TypeError("Todos os tipos de campo em Tuple devem ser instâncias de BaseField.")
    ...
    def get_cql_definition(self) -> str:
        ...
        inner_types = ', '.join(field_type.get_cql_definition() for field_type in self.field_types)
    ...
    def to_python(self, value: Any) -> Any:
        ...
        if isinstance(value, tuple):
            ...
            if len(value) != len(self.field_types):
                raise ValueError(f"Tuple esperava {len(self.field_types)} elementos, mas recebeu {len(value)}")
            ...
            for i, (val, field_type) in enumerate(zip(value, self.field_types)):
                try:
                    ...
                    raise ValueError(f"Erro ao converter elemento {i} da tupla: {e}") from e
    ...
    def get_pydantic_type(self) -> Type[Any]:
        ...
        inner_types = [field_type.get_pydantic_type() for field_type in self.field_types]
...
class UserDefinedType(BaseField):
    ...
    def get_cql_definition(self) -> str:
        ...
        type_name = getattr(self.udt_class, '__type_name__', self.udt_class.__name__.lower())
    ...
    def to_python(self, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            # Criar instância do UDT a partir do dicionário
            return self.udt_class(**value)
        raise TypeError(f"Não foi possível converter {value!r} para {self.udt_class.__name__}")
