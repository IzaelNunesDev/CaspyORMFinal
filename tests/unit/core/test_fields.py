...
from src.caspyorm.core.fields import (
...
class TestMap:
    ...
    def test_map_initialization_invalid_fields(self):
        ...
        with pytest.raises(TypeError):
            Map("invalid", Integer())  # type: ignore
        with pytest.raises(TypeError):
            Map(Text(), "invalid")  # type: ignore
...
class TestTupleField:
    """Testes para o campo Tuple."""
    
    def test_tuple_creation(self):
        """Testa a criação de um campo Tuple."""
        from src.caspyorm.core.fields import Tuple, Text, Integer, Boolean
        
        # Tuple simples
        tuple_field = Tuple(Text(), Integer())
        assert len(tuple_field.field_types) == 2
        assert isinstance(tuple_field.field_types[0], Text)
        assert isinstance(tuple_field.field_types[1], Integer)
        
        # Tuple com múltiplos tipos
        complex_tuple = Tuple(Text(), Integer(), Boolean(), Text())
        assert len(complex_tuple.field_types) == 4
    
    def test_tuple_empty_error(self):
        """Testa erro ao criar Tuple sem tipos."""
        from src.caspyorm.core.fields import Tuple
        
        with pytest.raises(TypeError, match="Tuple deve ter pelo menos um tipo de campo"):
            Tuple()
    
    def test_tuple_invalid_field_type(self):
        """Testa erro ao usar tipo inválido em Tuple."""
        from src.caspyorm.core.fields import Tuple
        
        with pytest.raises(TypeError, match="Todos os tipos de campo em Tuple devem ser instâncias de BaseField"):
            Tuple("text", "int")  # type: ignore
    
    def test_tuple_cql_definition(self):
        """Testa geração de definição CQL para Tuple."""
        from src.caspyorm.core.fields import Tuple, Text, Integer, Boolean
        
        tuple_field = Tuple(Text(), Integer(), Boolean())
        assert tuple_field.get_cql_definition() == "tuple<text, int, boolean>"
        
        complex_tuple = Tuple(Text(), Integer(), Boolean(), Text())
        assert complex_tuple.get_cql_definition() == "tuple<text, int, boolean, text>"
    
    def test_tuple_to_python(self):
        """Testa conversão para Python."""
        from src.caspyorm.core.fields import Tuple, Text, Integer, Boolean
        ...
        # Conversão válida
        result = tuple_field.to_python(("hello", 42, True))
        assert result == ("hello", 42, True)
        
        # Conversão com tipos diferentes
        result = tuple_field.to_python(("world", "123", False))
        assert result == ("world", 123, False)
        
        # None
        assert tuple_field.to_python(None) is None
    
    def test_tuple_to_python_wrong_length(self):
        """Testa erro ao converter tupla com tamanho errado."""
        from src.caspyorm.core.fields import Tuple, Text, Integer
        
        tuple_field = Tuple(Text(), Integer())
        
        with pytest.raises(ValueError, match="Tuple esperava 2 elementos, mas recebeu 1"):
            tuple_field.to_python(("hello",))
        
        with pytest.raises(ValueError, match="Tuple esperava 2 elementos, mas recebeu 3"):
            tuple_field.to_python(("hello", 42, "extra"))
    
    def test_tuple_to_python_invalid_type(self):
        """Testa erro ao converter tipo inválido."""
        from src.caspyorm.core.fields import Tuple, Text, Integer
        
        tuple_field = Tuple(Text(), Integer())
        
        with pytest.raises(TypeError, match="Não foi possível converter"):
            tuple_field.to_python("not a tuple")
    
    def test_tuple_to_cql(self):
        """Testa conversão para CQL."""
        from src.caspyorm.core.fields import Tuple, Text, Integer, Boolean
        ...
        # Conversão válida
        result = tuple_field.to_cql(("hello", 42, True))
        assert result == ("hello", 42, True)
        
        # None
        assert tuple_field.to_cql(None) is None
    
    def test_tuple_to_cql_wrong_length(self):
        """Testa erro ao converter para CQL com tamanho errado."""
        from src.caspyorm.core.fields import Tuple, Text, Integer
        
        tuple_field = Tuple(Text(), Integer())
        
        with pytest.raises(ValueError, match="Tuple esperava 2 elementos, mas recebeu 1"):
            tuple_field.to_cql(("hello",))
    
    def test_tuple_get_pydantic_type(self):
        """Testa obtenção do tipo Pydantic."""
        from src.caspyorm.core.fields import Tuple, Text, Integer, Boolean
        from typing import Tuple as TypingTuple
        
        tuple_field = Tuple(Text(), Integer(), Boolean())
        pydantic_type = tuple_field.get_pydantic_type()
        
        # Verificar que é um tipo de tupla
        assert "Tuple" in str(pydantic_type)


class TestUserDefinedTypeField:
    """Testes para o campo UserDefinedType."""
    
    def test_udt_creation(self):
        """Testa a criação de um campo UserDefinedType."""
        from src.caspyorm.core.fields import UserDefinedType, Text, Integer
        from src.caspyorm.types.usertype import UserType
        
        # Criar uma classe UDT de teste
        class Address(UserType):
            street: Text = Text()
            city: Text = Text()
            zip_code: Text = Text()
        
        udt_field = UserDefinedType(Address)
        assert udt_field.udt_class == Address
        assert udt_field.python_type == Address
    
    def test_udt_invalid_class(self):
        """Testa erro ao usar classe inválida."""
        from src.caspyorm.core.fields import UserDefinedType
        
        with pytest.raises(TypeError, match="UserDefinedType deve receber uma classe que herda de UserType"):
            UserDefinedType(str)
        
        with pytest.raises(TypeError, match="UserDefinedType deve receber uma classe que herda de UserType"):
            UserDefinedType("not a class")  # type: ignore
    
    def test_udt_cql_definition(self):
        """Testa geração de definição CQL para UserDefinedType."""
        from src.caspyorm.core.fields import UserDefinedType, Text, Integer
        from src.caspyorm.types.usertype import UserType
        ...
        udt_field = UserDefinedType(Address)
        assert udt_field.get_cql_definition() == "frozen<address>"
        
        # Com nome personalizado
        class CustomAddress(UserType):
            __type_name__ = "custom_address"
            street: Text = Text()
            city: Text = Text()
        
        custom_udt_field = UserDefinedType(CustomAddress)
        assert custom_udt_field.get_cql_definition() == "frozen<custom_address>"
    ...
    def test_udt_to_python(self):
        ...
        # Conversão de instância
        address = Address(street="123 Main St", city="New York", zip_code="10001")
        result = udt_field.to_python(address)
        assert result == address
        
        # Conversão de dicionário
        data = {"street": "456 Oak Ave", "city": "Los Angeles", "zip_code": "90210"}
        result = udt_field.to_python(data)
        ...
        # None
        assert udt_field.to_python(None) is None
    
    def test_udt_to_python_invalid_type(self):
        """Testa erro ao converter tipo inválido."""
        from src.caspyorm.core.fields import UserDefinedType, Text
        from src.caspyorm.types.usertype import UserType
        ...
        with pytest.raises(TypeError, match="Não foi possível converter"):
            udt_field.to_python("not a dict or address")
    
    def test_udt_to_cql(self):
        """Testa conversão para CQL."""
        from src.caspyorm.core.fields import UserDefinedType, Text, Integer
        from src.caspyorm.types.usertype import UserType
        ...
        # Conversão de instância
        address = Address(street="123 Main St", city="New York", zip_code="10001")
        result = udt_field.to_cql(address)
        assert result == {"street": "123 Main St", "city": "New York", "zip_code": "10001"}
        
        # Conversão de dicionário
        data = {"street": "456 Oak Ave", "city": "Los Angeles", "zip_code": "90210"}
        result = udt_field.to_cql(data)
        assert result == data
        
        # None
        assert udt_field.to_cql(None) is None
    
    def test_udt_to_cql_invalid_type(self):
        """Testa erro ao converter para CQL tipo inválido."""
        from src.caspyorm.core.fields import UserDefinedType, Text
        from src.caspyorm.types.usertype import UserType
        ...
        with pytest.raises(TypeError, match="Não foi possível converter"):
            udt_field.to_cql("not a dict or address")
    
    def test_udt_get_pydantic_type(self):
        """Testa obtenção do tipo Pydantic."""
        from src.caspyorm.core.fields import UserDefinedType, Text
        from src.caspyorm.types.usertype import UserType
        ...
        udt_field = UserDefinedType(Address)
        pydantic_type = udt_field.get_pydantic_type()
        
        assert pydantic_type == Address 


class TestCollectionOfUDT:
    """Testes para coleções de UDTs."""
    def setup_method(self):
        from src.caspyorm.core.fields import Text, UserDefinedType
        from src.caspyorm.types.usertype import UserType
        class Address(UserType):
            street: Text = Text()
            city: Text = Text()
    ...
    def test_list_of_udt_cql(self):
        from src.caspyorm.core.fields import List, UserDefinedType
        field = List(UserDefinedType(self.Address))
        assert field.get_cql_definition() == "list<frozen<address>>"

    def test_set_of_udt_cql(self):
        from src.caspyorm.core.fields import Set, UserDefinedType
        field = Set(UserDefinedType(self.Address))
        assert field.get_cql_definition() == "set<frozen<address>>"

    def test_map_of_udt_cql(self):
        from src.caspyorm.core.fields import Map, Text, UserDefinedType
        field = Map(Text(), UserDefinedType(self.Address))
        assert field.get_cql_definition() == "map<text,frozen<address>>"

    def test_list_of_udt_to_python_and_cql(self):
        from src.caspyorm.core.fields import List, UserDefinedType
        addr1 = self.Address(street="Rua 1", city="Cidade 1")
        addr2 = self.Address(street="Rua 2", city="Cidade 2")
        field = List(UserDefinedType(self.Address))
        # Para Python
        result = field.to_python([
            {"street": "Rua 1", "city": "Cidade 1"},
            {"street": "Rua 2", "city": "Cidade 2"}
        ])
        assert isinstance(result[0], self.Address)
        assert result[0].street == "Rua 1"
        # Para CQL
        cql = field.to_cql([addr1, addr2])
        assert cql == [addr1.model_dump(), addr2.model_dump()]

    def test_set_of_udt_to_python_and_cql(self):
        from src.caspyorm.core.fields import Set, UserDefinedType
        addr1 = self.Address(street="Rua 1", city="Cidade 1")
        addr2 = self.Address(street="Rua 2", city="Cidade 2")
        field = Set(UserDefinedType(self.Address))
        ...
        assert any(a.street == "Rua 1" for a in result)
        # Para CQL
        cql = field.to_cql({addr1, addr2})
        assert {tuple(sorted(d.items())) for d in cql} == {tuple(sorted(addr1.model_dump().items())), tuple(sorted(addr2.model_dump().items()))}

    def test_map_of_udt_to_python_and_cql(self):
        from src.caspyorm.core.fields import Map, Text, UserDefinedType
        addr1 = self.Address(street="Rua 1", city="Cidade 1")
        addr2 = self.Address(street="Rua 2", city="Cidade 2")
        field = Map(Text(), UserDefinedType(self.Address))
        # Para Python
        result = field.to_python({
            "a": {"street": "Rua 1", "city": "Cidade 1"},
            "b": {"street": "Rua 2", "city": "Cidade 2"}
        })
        assert result["a"].street == "Rua 1"
        # Para CQL
        cql = field.to_cql({"a": addr1, "b": addr2})
        assert cql["a"] == addr1.model_dump()
        assert cql["b"] == addr2.model_dump() 